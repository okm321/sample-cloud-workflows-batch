...
実務で組んだ構成を題材にした[ブログ記事](#) (TODO: 公開後にリンク差し替え) のサンプルレポジトリです。

> **Note**
> 各 child workflow の処理本体 (Cloud Run Jobs 呼び出し) は **`sys.log` で代替しています**。Workflows の構造 (parent/child, parallel, try/except, sleep) を `terraform apply` 直後から動作確認できるようにするためです。本番では各 `call: sys.log` を `call: googleapis.run.v1.namespaces.jobs.run` 等に置き換えてください。
>
> また、`failure-pattern` のデモのために、child workflow は `argument: {should_fail: true}` を受け取ると意図的に `raise` する作りになっています。

## 何が入っているか

- **2種類の parent workflow**: 成功パターン (try/except なし) と失敗パターン (try/except あり)
- **5種類の child workflow**: 日次集計の00:00待ち、parallel.branches での並列実行などの典型パターン
- **Terraform 一式**: Workflows / Service Account / IAM / API有効化までまとめて作成 (Scheduler は含めず、手動実行で動作確認する構成)
...
- Google Cloud プロジェクト
- `gcloud auth application-default login` 済み (もしくは適切な認証設定)
- Terraform 1.5+ がインストール済み
- 課金が有効化されたプロジェクト (Workflows API に必要)
